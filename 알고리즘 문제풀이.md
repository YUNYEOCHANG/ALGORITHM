...
![최소힙](알고리즘 문제풀이.assets/최소힙.png)

```python
import sys
import heapq as hq ------------- A
sys.stdin = open("input.txt", "r")
...
while True:
    ...
    if n == 0:
        if len(arr) == 0:
            ...
            print(hq.heappop(arr))    ------- B
    else:
        hq.heappush(arr, n)  -------- C

힙(heapq) 라이브러리와 관련된 문제다. 힙의 문법(import, heappop,heappush)과 개념을 숙지필요. heapq는 기본적으로 '최소힙'으로 작동한다.
...
while True:
    ...
    if n == 0:
        if len(arr) == 0:
            ...
            print(-hq.heappop(arr))     --- A
    else:
        hq.heappush(arr, - n)  --------  B

heapq는 기본적으로 최소힙으로 작동한다. push와pop이 최소값으로 트리를 구성하게 작동한다는 의미이다. 반대로 최대 힙으로 트리를 짜려면 ---A 와 --- B처럼 '-' 부호만 더해주면 된다. 즉, 예를들어 입력값으로 3, 4의 경우 최대값은 4이지만 최대힙을 따로 구현하는 코드를 작성하는 것보다, 최소힙을 기준으로 -3 -4 입력값을 받을 경우 -3보다 -4가 더 작으므로 최소힙 트리에서 -4가 루트가 된다. 결국 출력할 때 '-' 부호만 더하면 양의 정수로 바꿔서 표현할 수 있다.
```
...
전위,중위,후위 순회의 개념을 익히고, 스택에 주소값이 들어가면서 어떤식으로 출력이 되는지 이해하는 것이 중요하다. 
```
...
```python
.join의 문법을 활용해서 아래와 같이 개행할 것을 print("\n".join(ans))으로 한줄로 표현할 수 있다.
...
![최소공배수](알고리즘 문제풀이.assets/최소공배수.png)
...
최소공배수(lcm), 최대공약수(gcd)를 구하는 방법은 여러가지 있다. 위 세가지가 대표적인 코드다. 
```
...
![진수](알고리즘 문제풀이.assets/진수.png)

```python
n = list(input())
total_sum = 0
for i in range(len(n)-1, -1, -1):  # 0,1,2,3,4,5,6,7
    if n[i] == '1':
        total_sum += 2 ** ((len(n)-1) - i)
...
print(ans)
---------------------------
print(oct(int(input(),2))[2:]) ------ A

두 가지 모두 정답을 출력하는 코드이지만, --A 코드는 파이썬 내장함수를 이용하면서 시간초과가 나오지 않은 코드다. 

n진수 -> 10진수
print(int('101'), n) 문법은 입력값으로 받은 n진수를 10진수로 변환할 때 쓰는 문법이다. 
ex) print(int('202'),3) --> (3**0) * 2  + (3**1) * 0 + (3**2) * 2 == 20
...
bin,oct,hex는 각각 2진수,8진수,16진수이다. 
```
...
![진수진수](알고리즘 문제풀이.assets/진수진수.png)
...
for i in range(1000, 10000):
    ten_num = change(i, 10)
    twel_num = change(i, 12)
    sixteen_num = change(i, 16)

    if ten_num == twel_num and sixteen_num == ten_num:
        print(i)
-------------------------

진수와 관련된 문제다. 입력받은 10진수를 X진수로 바꿀 때는 ---A 코드를 사용하는 방법을 사용한다. 이때 ---A에 보여지는 매개변수에 주목해야한다. a는 10진수, b는 바꾸고자하는 진수를 의미하는데, b가 없으면 단순히 10진수를 각 자리수의 합을 더하는 함수일 뿐이다. 따라서 바꾸고자 하는 진수가 변화함으로 b값을 설정해줘야 한다.
```
...
![3의배수](알고리즘 문제풀이.assets/3의배수.png)

```
3의 배수는 각 자리수의 합이 3의 배수일 경우를 해당된다. 
```
...
![리플레이스](알고리즘 문제풀이.assets/리플레이스.png)

```python
words = input()
target = input()
ans = words.replace(target, '*')
print(ans.count('*'))
...
첫번째 코드는 replace 문법을 이용해 간단하게 풀이를 도출한 경우다. replace는 특정한 문자를 원하는 문자로 바꿀 때 사용한다. 이 문제에서 replace를 사용할 수 있었던 이유는 문제에 제시 된 것 처럼 중복을 허용하지 않았기 때문이다. 중복을 허용했다면 replace를 통해 바꾼 문자와 문자사이에 정답에 해당하는 경우를 제외할 수 있다. 
```
...
for i in range(len(prime_arr)):
    for j in range(i+1, len(prime_arr)):
        if prime_arr[i] * prime_arr[j] == n:
            if prime_arr[i] < k or prime_arr[j] < k:
                print('BAD', min(prime_arr[i], prime_arr[j]))
            else:
                print('GOOD')
...
'소수 * 소수 = 소수' 라는 점이 이 문제의 핵심이다. 첫번째 코드는 정답은 맞지만, 시간초과가 난 코드다. 아래의 코드는 소수와 소수의 곱은 소수라는 점을 개념을 이용해 짧은 코드로 정답을 도출한 코드다. 
````
